...
export class AppiumDriver {
    ...
    */
    ...
    public async compareElement(element: UIElement, imageName: string, tollerance: number = 0.01, timeOutSeconds: number = 3, tolleranceType?: ImageOptions) {
        return await this.compareRectangle(await element.getActualRectangle(), imageName, timeOutSeconds, tollerance, tolleranceType);
    }

    public async compareRectangle(rect: IRectangle, imageName: string, timeOutSeconds: number = 3, tollerance: number = 0.01, tolleranceType?: ImageOptions) {
        return await this.compare(imageName, timeOutSeconds, tollerance, rect, tolleranceType);
    }

    public async compareScreen(imageName: string, timeOutSeconds: number = 3, tollerance: number = 0.01, tolleranceType?: ImageOptions) {
        return await this.compare(imageName, timeOutSeconds, tollerance, undefined, tolleranceType);
    }

    private async compare(imageName: string, timeOutSeconds: number = 3, tollerance: number = 0.01, rect?: IRectangle, tolleranceType?: ImageOptions) {
        ...
        let result = await this._imageHelper.compareImages(pathActualImage, pathExpectedImage, pathDiffImage, tollerance, tolleranceType);
        ...
        if (!result) {
            ...
            while ((Date.now().valueOf() - eventStartTime) <= timeOutSeconds && !result) {
                ...
                await this.prepareImageToCompare(pathActualImage, rect);
                result = await this._imageHelper.compareImages(pathActualImage, pathExpectedImage, pathDiffImage, tollerance, tolleranceType);
                counter++;
            }
        } else {
