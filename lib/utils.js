...
function getReportPath(args) {
    var storage = createStorageFolder(resolve(args.projectDir, args.testFolder), "reports");
    var appName = args.appiumCaps.app.substring(args.appiumCaps.app.lastIndexOf("/") + 1, args.appiumCaps.app.lastIndexOf("."));
    storage = createStorageFolder(storage, appName);
    storage = createStorageFolder(storage, args.appiumCaps.deviceName);
    return storage;
...
;
function calculateOffset(direction, y, yOffset, x, xOffset, isIOS) {
    var speed = 10;
    var yEnd = (Math.abs(yOffset) + y);
    var xEnd = Math.abs(xOffset - x);
    ...
    console.log("ENDPOINT", { point: new point_1.Point(xEnd, yEnd), duration: duration });
 ...
 * @param xOffset
 */
function scroll(wd, driver, direction, isIOS, y, x, yOffset, xOffset) {
        ...
        return tslib_1.__generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    endPoint = calculateOffset(direction, y, yOffset, x, xOffset, isIOS);
